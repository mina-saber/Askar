...
export default function ProductCard({ product, showSalePercent = false }) {
  ...
  return (
    <>
      <div className="group flex flex-col cursor-pointer" id={`product-${product.id}`}>
        <div className="relative aspect-[3/4] mb-4 overflow-hidden bg-zinc-100 rounded-xl" onClick={handleNavigate}>
          ...
          {showSalePercent && product.is_sale && product.discount_percentage ? (
            <div className="absolute top-4 left-4 bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full pointer-events-none">
            ...
            <>
              ...
              {product.is_sale && (
                <div className="absolute top-4 right-4 bg-black text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full pointer-events-none">
                  Sale
                </div>
              )}
            </>
        ...
        <div className="flex flex-col flex-1 text-center" onClick={handleNavigate}>
          ...
          <div className="mt-auto flex justify-center items-center space-x-3">
            {product.is_sale && product.sale_price ? (
              <>
                <span className="text-rose-600 font-bold">${salePrice}</span>
      ...
      {isQuickViewOpen && (
        ...
        >
          ...
          >
            <button 
              onClick={closeQuickView}
              className="absolute top-4 right-4 z-10 w-8 h-8 flex items-center justify-center bg-white rounded-full text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 shadow-sm transition-colors"
            >
              <X size={18} />
            </button>
            ...
            <div className="md:w-1/2 p-8 md:p-10 flex flex-col">
              ...
              <div className="flex items-center space-x-3 mb-6 pb-6 border-b border-zinc-100">
                {product.is_sale && product.sale_price ? (
                  <>
                    <span className="text-2xl font-black text-rose-600">${salePrice}</span>
                    <span className="text-lg font-bold text-zinc-400 line-through">${price}</span>
                  </>
                ) : (
